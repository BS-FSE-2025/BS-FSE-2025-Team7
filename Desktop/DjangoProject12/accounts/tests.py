...
class ReportIssueViewTests(TestCase):
    def setUp(self):
        ...
        self.client = Client()
        self.url = reverse('report_issue')  # Adjust to your actual URL name for the report_issue view

    def test_report_issue_get_request(self):
        """Test the GET request for the report_issue view."""
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'report_problem.html')  # Replace with your actual template name
